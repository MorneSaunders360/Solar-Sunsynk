...
class SunsynkDataUpdateCoordinator(DataUpdateCoordinator):
    ...
    async def _async_update_data(self):
        ...
        try:
                ...
                try:
                    ...
                    try:
                        ...
                        if etoday == 0:
                            ...
                            dailyUsed = float(_inverter_load_data[0].get("dailyUsed", 0) or 0)
                        ...
                        inverterdata.update({
                            "Solar Production": _inverter_input_data[0].get("etoday", 0),
                            "Solar to Battery": _inverter_battery_data[0].get("etodayChg", 0),
                            "Solar to Grid": _inverter_grid_data[0].get("etodayTo", 0),
                            "Solar to Load": Solar_to_Load,
                            "Total Load": _inverter_load_data[0].get("dailyUsed", 0),
                            "Grid to Load": Grid_to_Load,
                            "State of Charge": _inverter_battery_data[0].get("soc", 0),
                            "Charge": _inverter_battery_data[0].get("etodayChg", 0),
                            "Discharge": _inverter_battery_data[0].get("etodayDischg", 0),
                            "Instantaneous Grid I/O Total": _inverter_grid_data[0].get("limiterTotalPower", 0),
                            "Instantaneous Generation": _inverter_input_data[0].get("pac", 0),
                            "Instantaneous Battery SOC": _inverter_battery_data[0].get("bmsSoc", 0),
                            "Instantaneous Battery I/O": _inverter_battery_data[0].get("power", 0),
                            "Instantaneous Load": _inverter_load_data[0].get("totalPower", 0),
                        ...
                        _LOGGER.error(
                            ...
                            invertor, index_error, jsondata[invertor]
                        )
                        continue
                    except Exception as stats_error:
                        _LOGGER.error(
                            "Unexpected error while processing statistics for inverter %s: %s",
                            invertor, stats_error
                        )
                        continue

                    
                except KeyError as key_error:
                    _LOGGER.error("Missing key in data for inverter %s: %s", invertor, key_error)
                    continue
            ...
            _LOGGER.error("Client response error while fetching data: %s", response_error)
